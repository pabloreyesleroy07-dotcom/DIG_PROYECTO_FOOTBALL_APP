...
class FootballAsyncEngine:
    ...
    async def execute_async(self, day="tomorrow"):
        """
        ...
        :param day: Input date mapping (today, tomorrow, YYYY-MM-DD).
        :return: Success/failure message JSON dictionary payload.
        """
        # Match Target Date
        if day == "today": target_date = datetime.now().strftime("%Y-%m-%d")
        elif day == "tomorrow": target_date = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
        else:
            try:
